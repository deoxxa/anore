...
Model.prototype.set = function set(key, value, options) {
  ...
  if (this._bubbleEvents) {
    ...
    var onRemoved = function onRemoved(parent, oldKey) {
      if (parent === self && oldKey === key) {
        value.off("change", onChange);
        value.off("removed", onRemoved);
      }
    };

    value.on("change", onChange);
    value.on("removed", onRemoved);
  }
  ...
  // notify listeners of the old value that it's been removed

  if (previousValue && !options.silent) {
    previousValue.emit("removed", this, key);
  }
  ...
  if (!options.silent) {
    value.emit("added", this, key);
  }

  // tell listeners of the new value that it's replaced another value

  if (previousValue && !options.silent) {
    value.emit("replace", this, key, previousValue);
  }
