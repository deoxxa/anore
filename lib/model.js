...
Model.prototype.set = function set(key, value, options) {
  ...
  // handle changing primitives

  if (previousValue instanceof Primitive && (typeof value !== "object" || value === null)) {
  ...
  if (previousValue instanceof Collection && Array.isArray(value)) {
    previousValue.setElements(value);

    return this;
  }

  // handle changing models

  if (previousValue instanceof Model && typeof value === "object" && value !== null && !Array.isArray(value) && !(value instanceof Primitive || value instanceof Model || value instanceof Collection)) {
    previousValue.multiSet(value, {
      absolute: true,
  ...
  // if we're at this point, we've exhausted our options for pushing the
  // responsibility for changes down into nested properties. thus, we box things
  // that need boxing.
